...
export async function apply(ctx) {
  ...
  // ── /api/hot-mcp 精确 Fetch 路由（供浏览器设置页 UI 调用）─────────
  // 新 dsh（>= 0.1.5-rc.1）里 connection.rpc.handle 会把通道路由注册在
  // owner.webServer 上（owner 经 Cordis traceable/shadow 绑定到 connection
  // 插件的 fiber），而 connection 插件自身的 inject 已从 ['webServer'] 改为
  // ['credentials']（webServer 只在内部 ctx.inject(['webServer'], …) 派生上下文
  // 里注入、仅用于内置 /api），于是任何插件调用 rpc.handle 都会在加载时抛
  // `cannot get property "webServer" without inject`。因此改用
  // connection.fetch.register：它不读取 webServer，且走 /api 载体、继承框架的
  // Host/Origin 栅栏 + 浏览器会话认证（/api 分发时精确路由优先于 gateway 拦截器）。
  ...
  if (connection?.fetch) {
    ...
    ctx.effect(() => connection.fetch.register({
      ...
      async fetch(request) {
        ...
        const result = await rpcHandler(envelope?.method, envelope?.payload ?? {});
        return Response.json({ type: "server-response", rpcId: envelope?.rpcId, result });
      },
    }), "hot_mcp: /api/hot-mcp RPC route");
  }
}
